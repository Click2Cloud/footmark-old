    ...
    def create_rds_instance(self, db_engine, engine_version, db_instance_class, db_instance_storage,
        ...
        try:
            ...
            if (ex.args is None) or (ex.args == "need more than 2 values to unpack") \
                ...
                results.append({"Error Message": "The API is showing None error code and "
                                                 "error message while creating rds instance"})
            elif error_code in error_dict:
                results.append("Following error occur while creating rds instance: " + str(error_dict[error_code]))
            else:
                results.append("Failed to create rds instance with error code " + error_code +
        ...
        try:
            ...
            if (ex.args is None) or (ex.args == "need more than 2 values to unpack") \
                ...
                results.append({"Error Message": "The API is showing None error code and error"
                                                 " message while changing rds instance type"})
            elif error_code in error_dict:
                results.append("Following error occur while changing rds instance type: " + str(error_dict[error_code]))
            else:

                results.append("Failed to change rds instance type with error code " + error_code +
                               " and message: " + error_msg)
